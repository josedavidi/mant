    ...
    <div class="container">
      <div class="row">
        <div class="col-sm-12">
          ...
          <tbody>
            <?php 
            $procesoMes= $db->query("SELECT m.nombre as mn, p.nombre as pn, s.nombre as sn, ss.nombre as ssn, pm.estado as pme, eq.nombre as eqn FROM proceso_mes pm INNER JOIN meses m ON pm.id_mes=m.id INNER JOIN procesos p ON pm.id_proceso=p.id INNER JOIN sistemas s ON s.id=p.id_sistema INNER JOIN subsistemas ss ON ss.id=p.id_subsistema INNER JOIN equipos eq ON eq.id=pm.id_equipo")or die('error'. mysqli_errno($db));
